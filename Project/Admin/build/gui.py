...
ASSETS_PATH = OUTPUT_PATH / Path(r"D:/college/GP/admin control/build/assets/frame0")
...
'''def delete_song_from_files(filename):
    try:
        os.remove(filename)
...
'''# Define the "Add Song" function
def add_song_to_player():
    # Show a file dialog to select a music file
    file_path = filedialog.askopenfilename(title="Select a song", filetypes=[("Audio files", "*.mp3")])
    if file_path:
        # Copy the selected music file to the "J:/music" folder
        shutil.copy(file_path, music_dir)
        print(f"{file_path} added to J:/music")
        # Copy the selected music file to the list
        playList.insert(END, os.path.join(music_dir, file_path))'''

def return_lyrics_for_database():
    song_lyrics = "this is a test for the lyrics of the song, hi hope you finsh the project soon"
    return song_lyrics
    
def return_explictLabel_for_database():
    song_lyrics = 1
    return song_lyrics
...
def add_song():
    def add_song_path():
        ...
        if song_name and artist_name and file_path:
            ...
            lyrics_text = return_lyrics_for_database()
            explict_label =  return_explictLabel_for_database()
...
icon_image = tk.PhotoImage(file="D:/college/GP/admin control/build/assets/frame0/image_16.png")
...
#show lyrics
def get_lyrics():
    lyrics = lyrics = "If I could choose anything I would be good and bad one day I would fall into love and stayWould fall into loveAnd it's not fairI keep on writing a sequel to storiesI know that are not thereI don't wanna die but I don't wanna live like thisI just wanna feel something, I just wanna feelI just wanna feel something, I just wanna feelSomething really real so that I can reallyFeel like a person againI just wanna feel something, I just wanna feelI just wanna feel something, I just wanna feeSomething really real so that I can reallyFeel like a person againIf I am telling the truthWatching my friends break their hearts into twoMakes me jealous, I know that it's cruelBut what can you do?And it's not fairI keep on writing a sequel to storiesI know that are not thereI don't wanna die but I don't wanna live like thisI just wanna feel something, I just wanna feelI just wanna feel something, I just wanna feelCan you tell me what's wrong with me?"
    return  lyrics
def show_lyrics():
    ...
    lyrics_label = tk.Label(lyrics_window, text=lyrics_formatted, font=("Arial", 12), bg="white", fg="black", justify="center")
